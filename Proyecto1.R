...
<<<<<<< HEAD
#============================================ Revision de ambos clusters===========================================

#revisamos el indice de correlacion, para eso definimos una funcion para realizarlo en ambos casos

correlacion <- function(data, modelo, clusters){
  
  distancias <- dist(data) %>% as.matrix()
=======
...
>>>>>>> f13798d74d5b0d092c813ada4e9218aaa8cd0ba6
...
<<<<<<< HEAD
  ...
  matriz_ideal <- matrix(0, nrow = nrow(data), ncol = nrow(data))
...
#en segundo lugar revisamos el indice de cohesion, aplicamos el mismo proceso que antes

cohesion <- function(data, modelo,clusters){
  ...
  for (i in 1:length(withinCluster)){
    ...
    tempData <- data[which(clusters == i),]
...
#luego se realiza el coeficiente de separacion, que se realiza de la siguiente forma

separacion <- function(data, modelo,clusters){
  centroide_total <- colMeans(data)
...
correlacion(data_tsne, modelo_dbscan, modelo_dbscan$cluster)
cohesion(data_tsne, modelo_dbscan, modelo_dbscan$cluster)
separacion(data_tsne, modelo_dbscan, modelo_dbscan$cluster)

correlacion(data_tsne, model_all, model_all$classification)
cohesion(data_tsne, model_all, model_all$classification)
separacion(data_tsne, model_all, model_all$classification)
